...
function sumByDate(posts: Post[]): Record<string, number> {
  ...
  for (const p of posts) {
    if (p.practiceMinutes) {
      const dateKey = p.createdAt.slice(0, 10) // "YYYY-MM-DD"
...
export default function ReportPage({ userPosts }: Props) {
  ...
  const longestSession = Math.max(
    ...sourcePosts.map((p) => p.practiceMinutes ?? 0),
    0
  )
    ...
    <>
      ...
      <main className="max-w-md mx-auto pb-24">
                    ...
                    {d.minutes > 0 && (
                      <div
                        className={`w-full rounded-t-sm transition-all ${
                          isToday ? "bg-rose-500" : "bg-rose-300"
                        }`}
                        style={{ height: `${height}%` }}
                        title={`${d.minutes}分`}
                      />
        ...
        <section className="mx-4 mt-3 rounded-2xl border border-gray-100 overflow-hidden">
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {p.createdAt.slice(0, 10).replace(/-/g, "/")} · @{p.user.username}
                  </p>
                </div>
          ...
          {sourcePosts.filter((p) => p.practiceMinutes).length === 0 && (
